...
opts.Add(BoolVariable("fast_unsafe",
         "Enable unsafe options for faster rebuilds", False))
opts.Add(BoolVariable("compiledb",
         "Generate compilation DB (`compile_commands.json`) for external tools", False))
...
if selected_platform in platform_list:
    ...
    if env.msvc:  # MSVC
        ...
        if env["warnings"] == "extra":
            env.Append(CCFLAGS=["-Wall", "-Wextra", "-Wwrite-strings",
                       "-Wno-unused-parameter"] + common_warnings)
    ...
    if env['platform'] in ['osx', 'android', 'iphone',"javascript"]:
...
def print_elapsed_time():
    ...
    print("[Time elapsed: {}.{:03}]".format(time.strftime(
        "%H:%M:%S", time.gmtime(elapsed_time_sec)), time_ms))
