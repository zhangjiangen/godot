...
template <typename F>
inline void generic_brush_op(Image &image, Image &brush, const Vector2 &p_pos, float factor, F op) {
...
#ifdef _DEBUG
	printf("Constructing ImageUtils");
#endif
}
...
Vector2 ImageUtils::get_red_range(Ref<Image> image_ref, const Rect2 &rect) const {
...
float ImageUtils::get_red_sum(Ref<Image> image_ref, const Rect2 &rect) const {
...
void ImageUtils::blur_red_brush(Ref<Image> image_ref, Ref<Image> brush_ref, const Vector2 &p_pos, float factor) {
	ERR_FAIL_COND(image_ref.is_null());
	ERR_FAIL_COND(brush_ref.is_null());
	Image &image = **image_ref;
	Image &brush = **brush_ref;
...
void ImageUtils::paint_indexed_splat(Ref<Image> index_map_ref, Ref<Image> weight_map_ref,
		Ref<Image> brush_ref, const Vector2 &p_pos, int texture_index, float factor) {
...
void ImageUtils::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("get_red_sum_weighted", "image", "brush_image", "pos", "factor"), &ImageUtils::get_red_sum_weighted);
	ClassDB::bind_method(D_METHOD("add_red_brush", "image", "brush_image", "pos", "factor"), &ImageUtils::add_red_brush);
	ClassDB::bind_method(D_METHOD("lerp_channel_brush", "image", "brush_image", "pos", "factor", "target_value", "channel"), &ImageUtils::lerp_channel_brush);
