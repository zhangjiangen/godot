...
std::vector<std::vector<BasicMesher::SplinePoint>> BasicMesher::get_splines(std::vector<Tree3DStem> &stems) {
	std::vector<std::vector<SplinePoint>> splines;

	for (Tree3DStem &stem : stems) {
		Tree3DNode *stem_node = &stem.node;
...
void BasicMesher::get_splines_rec(std::vector<std::vector<SplinePoint>> &splines, Tree3DNode *current_node, Vector3 current_position) {
...
void BasicMesher::mesh_spline(Tree3DMesh &mesh, std::vector<SplinePoint> &spline) {
	for (SplinePoint &spline_point : spline) {
		int n = mesh.vertices.size();
        Tree3DGeometry::add_circle(mesh.vertices, spline_point.position, spline_point.direction, spline_point.radius, radial_resolution);
...
Tree3DMesh BasicMesher::mesh_tree(Tree3D &tree) {
	std::vector<Tree3DStem> &tree_stems = tree.get_stems();
