...
// 获取树叶创建的结点信息
void get_leaf_candidates(Vector3 base_pos, Node &n, std::vector<LeavesCandidates> &stems, float max_radius) {
...
void get_leaf_candidates(Stem &s, std::vector<LeavesCandidates> &stems, float max_radius) {
	get_leaf_candidates(s.position, s.node, stems, max_radius);
}

void LeavesFunction::execute(std::vector<Stem> &stems, int id, int parent_id) {
