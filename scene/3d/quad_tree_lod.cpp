...
void QuadTreeLod::set_callbacks(const Callable &make_cb, const Callable &recycle_cb, const Callable &vbounds_cb) {
	_make_func = make_cb;
...
void QuadTreeLod::update(const Vector3 &view_pos) {
...
void QuadTreeLod::debug_draw_tree(Object *ci) {
	if (ci == nullptr) {
		return;
	}
	if (ci->get_class() == "CanvasItem") {
		if (ci != nullptr)
			_debug_draw_tree_recursive((CanvasItem *)ci, ROOT, _max_depth, 0);
	}
}
...
Variant QuadTreeLod::_make_chunk(int lod, int origin_x, int origin_y) {
	if (_make_func.is_valid()) {
		Variant args[] = { origin_x, origin_y, lod };
		const Variant *args_ptr[] = { &args[0], &args[1], &args[2] };
		Variant r;
		Callable::CallError error;
		_make_func.call(args_ptr, 3, r, error);
		return r;
	} else {
		return Variant();
	}
...
void QuadTreeLod::_recycle_chunk(unsigned int quad_index, int lod) {
	...
	if (_recycle_func.is_valid()) {
		Variant args[] = { quad->get_data(), quad->origin_x, quad->origin_y, lod };
		const Variant *args_ptr[] = { &args[0], &args[1], &args[2], &args[3] };
		Variant r;
		Callable::CallError error;
		_recycle_func.call(args_ptr, 4, r, error);
	}
}
...
void QuadTreeLod::_update(unsigned int quad_index, int lod, const Vector3 &view_pos) {
	...
	if (_vertical_bounds_func.is_valid()) {
		Variant args[] = { quad->origin_x, quad->origin_y, lod };
		const Variant *args_ptr[] = { &args[0], &args[1], &args[2] };
		Variant result;
		Callable::CallError error;
		_vertical_bounds_func.call(args_ptr, 3, result, error);
...
void QuadTreeLod::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_callbacks", "make_cb", "recycle_cb", "vbounds_cb"), &QuadTreeLod::set_callbacks);
	...
	ClassDB::bind_method(D_METHOD("debug_draw_tree", "ci"), &QuadTreeLod::debug_draw_tree);
}
