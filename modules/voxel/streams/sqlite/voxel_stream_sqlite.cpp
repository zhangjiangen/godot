						 ...
						 // .format(varray(_cache.get_indicative_block_count())));
						  ...
						  _cache.flush([con, &temp_data, &temp_compressed_data](VoxelStreamCache::Block &block) {
							  ...
							  if (block.has_voxels) {
								  if (block.voxels_deleted) {
									  ...
									  BlockSerializer::SerializeResult res =
											  BlockSerializer::serialize_and_compress(block.voxels);
							  ...
							  if (block.instances != nullptr) {
								  ...
								  ERR_FAIL_COND(!CompressedData::compress(to_span_const(temp_data),
										  temp_compressed_data, CompressedData::COMPRESSION_NONE));
							  }
