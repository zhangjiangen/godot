...
void GDProcMesh::_get_property_list(List<PropertyInfo> *p_list) const {
	...
	std::map<int, Ref<GDProcNode>>::const_iterator it;
	for (it = nodes.begin(); it != nodes.end(); it++) {
		PropertyInfo prop;
		String name = "nodes/";
		name += String::num_int64(it->first);
		prop.name = name;
		prop.type = Variant::OBJECT;
	...
	{
		PropertyInfo prop;
		prop.type = Variant::POOL_INT_ARRAY;
...
bool GDProcMesh::_get(const StringName &p_names, Variant &p_value) const {
	String p_name = p_names.operator String();
	...
	if (p_name.begins_with(str_nodes)) {
		...
		std::map<int, Ref<GDProcNode>>::const_iterator it = nodes.find(id);
		if (it == nodes.end()) {
		...
		for (it = nodes.begin(); it != nodes.end(); it++) {
			if (it->second->get_node_name() == input_name) {
				GDProcNode *p = (GDProcNode *)it->second.ptr();
				p_value = p->get_input();
				return true;
			}
		}
...
bool GDProcMesh::_set(const StringName &p_names, const Variant &p_value) {
	// temporary workaround to get around compile issue on Linux, needs further research
	String p_name = p_names.operator String();
	String str_nodes = String("nodes/");
	String str_inputs = String("inputs/");
...
void GDProcMesh::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("add_node"), &GDProcMesh::add_node);
	ClassDB::bind_method(D_METHOD("find_node"), &GDProcMesh::find_node);
	ClassDB::bind_method(D_METHOD("get_node"), &GDProcMesh::get_node);
	ClassDB::bind_method(D_METHOD("get_node_id_list"), &GDProcMesh::get_node_id_list);
	ClassDB::bind_method(D_METHOD("remove_node"), &GDProcMesh::remove_node);
	ClassDB::bind_method(D_METHOD("_child_name_changed"), &GDProcMesh::_child_name_changed);

	/* should add some methods to interact with nodes directly */

	/* connections */
	ClassDB::bind_method(D_METHOD("add_connection"), &GDProcMesh::add_connection);
	ClassDB::bind_method(D_METHOD("remove_connection"), &GDProcMesh::remove_connection);
	ClassDB::bind_method(D_METHOD("get_connection_list"), &GDProcMesh::get_connection_list);
